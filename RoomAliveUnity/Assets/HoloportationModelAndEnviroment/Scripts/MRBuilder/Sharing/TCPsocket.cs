...
namespace HoloGroup.Networking.Internal.Sockets
{
    ...
    private async void ConnectCompleted(IAsyncAction asyncInfo, AsyncStatus status)
    {
...
#else
        private void SocketListener_ConnectionReceived(IAsyncResult res)
        {
            TcpClient client = _socketListener.EndAcceptTcpClient(res);
            NetworkStream stream = client.GetStream();

            bool needCloseListener = _socketBehaviour.ListenerTransferingProcess(stream);
        ...
        private void ConnectCompleted(IAsyncResult res)
        {
            if (_socket.Connected)
            {
                NetworkStream stream = _socket.GetStream();
            ...
            else
            {
                _socket.Close();
                _socket = null;
                _socketBehaviour.SocketTransferingProcess(false);
            }
        }
#endif
